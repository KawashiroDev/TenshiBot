...
#portrait
    ...
    @commands.command()
    @commands.cooldown(rlimit_cmd, rlimit_time, commands.BucketType.user)
    async def Gengetsu(self, ctx):
        em = discord.Embed(title='', description=' ', colour=0x496DA9)
        char = 'gengetsu_(touhou)'
        await self.imagefetch(ctx, char, em, 0)

    @commands.command()
    @commands.cooldown(rlimit_cmd, rlimit_time, commands.BucketType.user)
    async def Mugetsu(self, ctx):
        em = discord.Embed(title='', description=' ', colour=0x496DA9)
        char = 'mugetsu_(touhou)'
        await self.imagefetch(ctx, char, em, 0)

    @commands.command()
    @commands.cooldown(rlimit_cmd, rlimit_time, commands.BucketType.user)
    async def rika(self, ctx):
        em = discord.Embed(title='', description=' ', colour=0x496DA9)
        char = 'rika_(touhou)'
        await self.imagefetch(ctx, char, em, 0)
...
#        await self.imagefetch(ctx, char, em, 0)
    ...
    async def chimi(self, ctx):
        ...
        embed_name = 'Character image'
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(verify_ssl=False)) as session:
            async with session.get('http://' + booru + '/index.php?page=dapi&s=post&q=index&api_key=' + g_api + '&user_id=' + g_user + '&tags=' + badtags_moderate + '+' + char) as r:
                if r.status == 200:
                    soup = BeautifulSoup(await r.text(), "lxml")
                    num = int(soup.find('posts')['count'])
                    maxpage = int(round(num / 100))
